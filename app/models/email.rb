...
class Email < ApplicationRecord
  ...
  def notify_slack
    ...
    SlackNotifier.notify(message_parts.join("\n"), channel: '#rich-lawns', username: 'Mail-Bot', icon_emoji: ':mailbox:')
  end
  ...
  def failure(*issues)
    ...
    SlackNotifier.notify("Failed to parse: \n* #{issues.join("\n* ")}\n<#{Rails.application.routes.url_helpers.email_url(id: id)}|Click here to view.>", channel: '#rich-lawns', username: 'Mail-Bot', icon_emoji: ':mailbox:') if issues.any?
  end
